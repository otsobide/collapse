...
use collapse_core::{
    compress, compress_dir, extract_with, Algorithm, ExtractOptions, NameRules, Verify,
};
...
///
/// Returns the names as written, which are the names the archive spells: a name
/// this host cannot hold fails the whole extraction rather than being adjusted
/// to fit, so there is nothing for the user to answer and nothing for this
/// command to take besides where the archive is and where it should go.
///
/// A naming refusal comes back as an ordinary `Err`. It used to have a variant
/// of its own so the dialog could stay open on it; there is no dialog now, and
/// what is left is a message the user reads in the same place as any other
/// failure.
#[tauri::command(async)]
pub fn extract_archive(archive: String, output_dir: String) -> Result<Vec<String>, String> {
    ...
    // The host's rules, spelled out rather than left to the default, so the
    // machine being judged is visibly this one.
    let options = ExtractOptions::new().with_rules(NameRules::host());
    extract_with(&archive_path, &output, &options).map_err(|e| e.to_string())
}
