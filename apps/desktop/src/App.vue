 ...
 *
 * A name this computer cannot write fails the whole extraction and arrives as
 * an ordinary error: nothing is on disk, and there is nothing to ask, so it
 * belongs in the banner with every other failure.
 */
async function runExtraction(archive, outputDir) {
  const files = await invoke('extract_archive', { archive, outputDir })
